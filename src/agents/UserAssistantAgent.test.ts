    ...
    try {
      ...
      try {
        webIntel = await this.performWebSearch(
          `${cveId} vendor dispute false positive rejected legitimate status verification`
        );
      } catch (error) {
      ...
      const isDisputed = webIntel?.summary ? this.isActualDispute(webIntel.summary, cveId) : false;
      
      // Check vendor confirmation
      const vendorConfirmation = await this.checkVendorConfirmation(cveId, webIntel);
      
      // Calculate confidence
      const { confidence, reasoning } = this.calculateValidationConfidence(
        cveData, epssData, webIntel, vendorConfirmation, isDisputed
      );
      ...
      if (isDisputed) {
        legitimacyStatus = 'DISPUTED/FALSE POSITIVE';
      ...
      // Generate response
      let responseText = `**${cveId} Validation Results**\n\n`;
      ...
      if (isDisputed) {
        responseText += `**⚠️ Dispute Evidence:**\n`;
        responseText += `This CVE appears to be disputed by vendors or flagged as a false positive by security tools.\n\n`;
      }

      // Provide recommendations
      responseText += `**💡 Recommendation:**\n`;
      if (legitimacyStatus === 'LEGITIMATE' || legitimacyStatus === 'LIKELY LEGITIMATE') {
        responseText += `• This appears to be a legitimate vulnerability\n`;
        responseText += `• Proceed with normal vulnerability management processes\n`;
        responseText += `• Check for available patches and assess risk to your environment\n`;
      } else if (legitimacyStatus === 'DISPUTED/FALSE POSITIVE') {
        responseText += `• This CVE appears to be disputed or flagged as false positive\n`;
        responseText += `• Verify through official vendor channels before taking action\n`;
        responseText += `• Consider this low priority unless confirmed otherwise\n`;
      } else {
        responseText += `• Legitimacy is uncertain - investigate further\n`;
        responseText += `• Check vendor security advisories directly\n`;
        responseText += `• Monitor for updates on this CVE's status\n`;
      }
